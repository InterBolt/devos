...
cmd.provision() {
  ...
  log_info "${vPROJECT_NAME} - Building SSH config."
  ...
  [[ ! -f "${caddy_dockerfile}" ]] && log_error "Could not find: ${caddy_dockerfile}" && exit 1
  [[ ! -f "${caddyfile}" ]] && log_error "Could not find: ${caddyfile}" && exit 1

  # Removes the linux login message for future SSH commands.
  log_info "${vPROJECT_NAME} - Disabling linux login message."
  lib.ssh.project_command 'touch /root/.hushlogin || exit 0'

  log_info "${vPROJECT_NAME} - Exposing web ports."
  lib.ssh.project_command 'ufw allow 22,80,443'
  lib.ssh.project_command 'ufw allow 443/udp'

  log_info "${vPROJECT_NAME} - Enabling color support."
  lib.ssh.project_command 'echo "export TERM=xterm-256color" >>/root/.bashrc'

  log_info "${vPROJECT_NAME} - Creating /root/solos/apps"
  lib.ssh.project_command 'mkdir -p /root/solos/apps'

  log_info "${vPROJECT_NAME} - Creating /root/solos/docker"
  lib.ssh.project_command 'mkdir -p /root/solos/docker'

  log_info "${vPROJECT_NAME} - Creating /root/solos/logs"
  lib.ssh.project_command 'mkdir -p /root/solos/logs'

  log_info "${vPROJECT_NAME} - Creating /root/solos/cache"
  lib.ssh.project_command 'mkdir -p /root/solos/cache'

  log_info "${vPROJECT_NAME} - Creating /root/solos/caddy"
  lib.ssh.project_command 'mkdir -p /root/solos/caddy/config'

  log_info "${vPROJECT_NAME} - Creating /root/solos/caddy/data"
  lib.ssh.project_command 'mkdir -p /root/solos/caddy/data'

  log_info "${vPROJECT_NAME} - Creating /root/solos/caddy/etc"
  lib.ssh.project_command 'mkdir -p /root/solos/caddy/etc'

  log_info "${vPROJECT_NAME} - Uploading Caddyfile."
  lib.ssh.project_rsync_up "${launch_dir}/Caddyfile" "/root/solos/caddy/etc/"

  log_info "${vPROJECT_NAME} - Uploading Dockerfile.caddy."
  ...
  log_info "${vPROJECT_NAME} - Installing Docker."
...
EOF
  ...
  log_info "${vPROJECT_NAME} - Building Caddy."
  ...
  log_info "${vPROJECT_NAME} - Starting caddy."
