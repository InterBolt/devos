...
apply_manifest.validate() {
  ...
  if [[ ! -f ${manifest_file} ]]; then
    shared.log_error "Manifest - does not exist at ${manifest_file}"
    return 1
  fi
  local manifest="$(cat ${manifest_file})"
  if [[ ! $(jq '.' <<<"${manifest}") ]]; then
    shared.log_error "Manifest - not valid json at ${manifest_file}"
    return 1
  fi
  ...
  for plugin_name in "${plugin_names[@]}"; do
    ...
    if [[ ! -f ${plugin_executable_path} ]]; then
      shared.log_warn "Manifest - no executable at ${plugin_executable_path}. Will treat like a missing plugin."
      missing_plugins+=("${plugin_name}" "${plugin_source}")
      i=$((i + 1))
      continue
    fi
    ...
    if [[ ${plugin_config_source} != "${plugin_source}" ]]; then
      shared.log_info "Plugin ${plugin_name} is CHANGING"
...
apply_manifest.commit_dirs() {
  ...
  for dir in "${dirs[@]}"; do
    ...
    if [[ -d ${plugin_path} ]]; then
      shared.log_warn "Manifest - plugin ${plugin_name} already exists at ${plugin_path}. Skipping."
      i=$((i + 1))
      continue
    fi
...
apply_manifest.create_plugins() {
  ...
  apply_manifest.commit_dirs "${plugins_dir}" "${plugin_tmp_dirs[*]}" "${plugin_names[*]}" || return 1
}
...
apply_manifest.main() {
  ...
  apply_manifest.validate "${plugins_dir}" >"${return_file}" || return 1
  ...
  if ! cp -rfa "${plugins_dir}"/. "${tmp_plugins_dir}"/; then
    shared.log_error "Manifest - unable to copy ${plugins_dir} to ${tmp_plugins_dir}"
    return 1
  fi
  ...
  shared.log_info "Manifest - found ${missing_count} missing plugins"
  shared.log_info "Manifest - found ${change_count} changed plugins"
  ...
  return_file="$(mktemp)"
  apply_manifest.validate "${tmp_plugins_dir}" >"${return_file}" || return 1
  ...
  if [[ ${#remaining_missing_plugins_and_sources[@]} -gt 0 ]]; then
    shared.log_error "Manifest - missing plugins: ${remaining_missing_plugins_and_sources[*]}"
    return 1
  fi
  shared.log_info "Manifest - added all missing plugins successfully"
  if [[ ${#remaining_changed_plugins_and_sources[@]} -gt 0 ]]; then
    shared.log_error "Manifest - changed plugins: ${remaining_changed_plugins_and_sources[*]}"
    return 1
  fi
  shared.log_info "Manifest - updated all changed plugins successfully"
