...
runtime_fn_arg_info "g-review" "Review a file using OpenAI's GPT-4"
runtime_fn_arg_accept 'f:' 'filepath' 'The file to review'
runtime_fn_arg_accept 'm?' 'model' 'OpenAI model' 'gpt-4'
runtime_fn_arg_accept 's?' 'specialty' 'A specialty to refine the system prompt. (eg. nextjs, vscode, etc)' ''
runtime_fn_arg_accept 'c?' 'context-file' 'A file whose contents we append to the user message.' ''
runtime_fn_arg_parse "$@"
filepath="$(runtime_fn_get_arg 'filepath')"
model="$(runtime_fn_get_arg 'model')"
specialty="$(runtime_fn_get_arg 'specialty')"
add_context_file="$(runtime_fn_get_arg 'context-file')"
