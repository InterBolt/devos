...
runtime_fn_arg_info "h-alias" "Build alias commands and save them into the .bashrc file"
...
for script_type in $(get_script_types); do
  ...
  for script_name in "scripts/aliased/$script_type"*.sh; do
  ...
  echo "alias s=\"source /root/.bashrc\"" >>~/.bashrc
  echo "$delimiter" >>~/.bashrc
done
...
alias_command="PREV_DIR=\$(pwd) && $runtime_repo_dir/scripts/lib/runtime.sh && cd $runtime_repo_dir/extension && npm run sync && cd \$PREV_DIR"
{
  echo "$delimiter"
  echo "alias ext='$alias_command'"
  echo "alias s=\"source /root/.bashrc\""
  echo "$delimiter"
} >>~/.bashrc
log.info "Run 's' to source the .bashrc"
