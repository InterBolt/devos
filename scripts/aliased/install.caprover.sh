...
runtime_fn_arg_info "i-caprover" "Installs caprover and prepares the one-click-app template for our postgres DB."
runtime_fn_arg_parse "$@"

runtime_fn_fail_on_used_port 80
  ...
  cat <<EOF
{
  ...
  "caproverOneClickApp": {
    ...
    "documentation": "https://github.com/interbolt/devos"
  }
}
EOF
)
echo "$database_json" >"$runtime_repo_dir/.secret.database.json"
