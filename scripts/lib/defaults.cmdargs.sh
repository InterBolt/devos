...
function cmdarg {
  ...
  local key="$2"
  if [[ "$shortopt" == "h" ]]; then
    log.error "-h is reserved for cmdarg usage"
    ${CMDARG_ERROR_BEHAVIOR} 1
  fi
  if [[ "$(type -t cmdarg_$key)" != "" ]] ||
    [[ "${CMDARG_FLAGS[$shortopt]}" != "" ]] ||
    [[ "${CMDARG_TYPES[$key]}" != "" ]]; then
    log.error "command line key '$shortopt ($key)' is reserved by cmdarg or defined twice"
    ${CMDARG_ERROR_BEHAVIOR} 1
  fi
  ...
  local argtype=${1:1:1}
  if [[ "$argtype" =~ ^[\[{]$ ]]; then
    log.error "Flags required [:?] when specifying Hash or Array arguments (${argtype})"
    ${CMDARG_ERROR_BEHAVIOR} 1
  elif [[ "$argtype" != "" ]]; then
    ...
    if [[ "${1:2:4}" == "[]" ]]; then
      ...
      if [[ $? -ne 0 ]]; then
        log.error 'Array variable '"${key}"' does not exist. Array variables MUST be declared by the user!'
        ${CMDARG_ERROR_BEHAVIOR} 1
      fi
      ...
      declare -p ${key} >/dev/null 2>&1
      if [[ $? -ne 0 ]]; then
        log.error 'Hash variable '"${key}"' does not exist. Hash variables MUST be declared by the user!'
        ${CMDARG_ERROR_BEHAVIOR} 1
      fi
  ...
  if [[ "$validatorfunc" != "" ]] && [[ "$(declare -F $validatorfunc)" == "" ]]; then
    log.error "Validators must be bash functions accepting 1 argument (not '$validatorfunc')"
    ${CMDARG_ERROR_BEHAVIOR} 1
  fi
...
function cmdarg_info {
  ...
  if [[ ! "$1" =~ $flags ]]; then
    log.error "cmdarg_info <flag> <value>" "Where <flag> is one of $flags"
    ${CMDARG_ERROR_BEHAVIOR} 1
  fi
  CMDARG_INFO["$1"]=$2
  set -u
...
function cmdarg_describe_default {
  ...
  case ${argtype} in
  $CMDARG_TYPE_STRING)
    echo "-${opt},--${longopt} <string>. ${description} ${default}"
    ;;
  $CMDARG_TYPE_BOOLEAN)
    echo "-${opt},--${longopt}[bool]. ${description} ${default}"
    ;;
  $CMDARG_TYPE_ARRAY)
    echo "-${opt},--${longopt} <array>. ${description}. Pass this argument multiple times for multiple values. ${default}"
    ;;
  $CMDARG_TYPE_HASH)
    echo "-${opt},--${longopt} <hash>. ${description}. Pass this argument multiple times for multiple key/value pairs. ${default}"
    ;;
  *)
    log.error "Unable to return string description for ${opt}; unknown type ${argtype}"
    ${CMDARG_ERROR_BEHAVIOR} 1
    ;;
  esac
  set -u
...
function cmdarg_validate {
  ...
  if [ "${CMDARG_VALIDATORS[$shortopt]}" != "" ]; then
    ...
    if [ $? -ne 0 ]; then
      log.error "Invalid value for -$shortopt : ${value}"
      ${CMDARG_ERROR_BEHAVIOR} 1
    fi
  fi
  return 0
...
function cmdarg_set_opt {
  ...
  $CMDARG_TYPE_HASH)
    ...
    if [[ "$k" == "$arg" ]] && [[ "$v" == "$arg" ]] && [[ "$k" == "$v" ]]; then
      log.error "Malformed hash argument: $arg"
      ${CMDARG_ERROR_BEHAVIOR} 1
    fi
    eval "$key[\$k]=\$v"
    ...
    ;;
  *)
    log.error "Unable to return string description for ${key}; unknown type ${CMDARG_TYPES[$key]}"
    ${CMDARG_ERROR_BEHAVIOR} 1
    ;;
  esac
  return 0
...
function cmdarg_parse {
  ...
  while [[ $# -ne 0 ]]; do
    ...
    if [[ "$fullopt" == "--" ]] && [[ $parsing -eq 0 ]]; then
      ...
      log.error "Malformed argument: ${fullopt}" "While parsing: $@"
      ${cmdarg_helpers['usage']} >&2
      ${CMDARG_ERROR_BEHAVIOR} 1
    fi

    if [[ "$opt" == "p" ]] || [[ "$longopt" == "print-help" ]]; then
      ${cmdarg_helpers['usage']} >&1
      exit 0
    fi
    ...
    if [ ${CMDARG["${opt}"]+abc} ]; then
      ...
      log.error "Unknown argument or invalid value : -${opt} | --${longopt}"
      ${cmdarg_helpers['usage']} >&2
      ${CMDARG_ERROR_BEHAVIOR} 1
    fi
  done
  ...
  if [ $failed -gt 0 ]; then
    if [[ "$missing" != "" ]]; then
      log.error "Missing arguments : ${missing}"
    fi
    echo >&2
    ${cmdarg_helpers['usage']} >&2
    ${CMDARG_ERROR_BEHAVIOR} 1
...
function cmdarg_traceback {
  ...
  for ((i = FRAMES - 2; i >= 1; i--)); do
    log.error '  File' \"${BASH_SOURCE[i + 1]}\", line ${BASH_LINENO[i]}, probably in ${FUNCNAME[i + 1]} "$(sed -n "${BASH_LINENO[i]}{s/^/    /;p}" "${BASH_SOURCE[i + 1]}")"
  done
  echo "  Error: $LASTERR"
  unset FRAMES
  set -u
