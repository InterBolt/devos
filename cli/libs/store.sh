...
lib.store._prompt() {
  ...
  if [[ -z ${input} ]]; then
    input="$(gum_bin input --cursor.foreground "#3B78FF" --prompt "Enter $1: ")"
    if [[ -z ${input} ]]; then
...
lib.store.global.del() {
  lib.store._del "${HOME}/.solos/${vSELF_LIB_STORE_DIRNAME}" "$@"
}

lib.store.global.get() {
  lib.store._get "${HOME}/.solos/${vSELF_LIB_STORE_DIRNAME}" "$@"
}

lib.store.global.set() {
  lib.store._set "${HOME}/.solos/${vSELF_LIB_STORE_DIRNAME}" "$@"
}

lib.store.project.del() {
  local project_dir="${HOME}/.solos/projects/${vPROJECT_NAME}"
  if [[ ! -d ${project_dir} ]]; then
    log_error "Store error: ${project_dir} does not exist."
    exit 1
  fi
