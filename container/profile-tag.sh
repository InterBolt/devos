...
profile_tag__base_dir="${HOME}/.solos/collections"
profile_tag__logs_dir="${profile_tag__base_dir}/logs"
profile_tag__config_dir="${profile_tag__base_dir}/config"
profile_tag__std_dir="${profile_tag__base_dir}/std"
...
collections dir: ${profile_tag__base_dir}
...
profile_tag.init_fs() {
  mkdir -p "${profile_tag__config_dir}"
  mkdir -p "${profile_tag__logs_dir}"
  mkdir -p "${profile_tag__std_dir}"
  if [[ ! -f "${profile_tag__config_dir}/categories" ]]; then
    {
      echo "<none>"
      echo "<create>"
    } >"${profile_tag__config_dir}/categories"
  fi
}
profile_tag.disgest_save() {
  local tmp_jq_output_file="$1"
  jq -c '.' <"${tmp_jq_output_file}" >>"${profile_tag__logs_dir}/tag.log"
  rm -f "${tmp_jq_output_file}"
}
profile_tag.create_tmp_files() {
  # This is used across several functions so must be global.
  profile_tag__working_tmp_dir="$(mktemp -d)"

  local stdout_file="${profile_tag__working_tmp_dir}/stdout"
  local stderr_file="${profile_tag__working_tmp_dir}/stderr"
  local cmd_file="${profile_tag__working_tmp_dir}/cmd"
  local return_code_file="${profile_tag__working_tmp_dir}/return_code"
  local stderr_tag_file="${profile_tag__working_tmp_dir}/stderr_tag"
  local stdout_tag_file="${profile_tag__working_tmp_dir}/stdout_tag"
...
profile_tag.digest() {
  ...
  if [[ -n "${cmd}" ]]; then
    ...
    mv "${stdout_file}" "${profile_tag__std_dir}/${tag_id}.out"
    mv "${stderr_file}" "${profile_tag__std_dir}/${tag_id}.err"
  ...
  profile_tag.disgest_save "${tmp_jq_output_file}"
}
profile_tag.trap_eval() {
  local tag_id="${1}"
  local cmd="${2}"
...
profile_tag.trap_lifeycle_scripts() {
  local lifecycle="${1}"
  local prompt="${2}"
...
profile_tag.trap() {
  ...
  if [[ ${BASH_COMMAND} = "profile_tag__trap_gate_open=t" ]]; then
    return 0
  fi
  ...
  # To do this, we unset some arbitrary variable (ie profile_tag__trap_gate_open) on the first trapped command, ls,
  ...
  if [[ -n "${profile_tag__trap_gate_open+set}" ]]; then
    unset profile_tag__trap_gate_open
    ...
    local blacklist="$(profile_tag.get_blacklist | xargs)"
    ...
    profile_tag.trap_lifeycle_scripts "preexec" "${submitted_prompt}"
    ...
    if [[ ${should_skip_tag} = false ]]; then
      ...
      profile_tag.create_tmp_files
      profile_tag.trap_eval "${tag_id}" "${submitted_prompt}"
      profile_tag.digest "${tag_id}"
    fi
    ...
    profile_tag.trap_lifeycle_scripts "postexec" "${submitted_prompt}"
  fi
...
profile_tag.install() {
  ...
  profile_tag.init_fs
  PROMPT_COMMAND='profile_tag__trap_gate_open=t'
  trap 'profile_tag.trap' DEBUG
}
profile_tag.apply_category() {
  local newline=$'\n'
  local category_choice="$(gum_tag_category_choice "${profile_tag__config_dir}/categories")"
...
profile_tag.main() {
  ...
  if [[ ${opt_note_only} = false ]]; then
    user_category="$(profile_tag.apply_category)"
  ...
  local tag_id="$(date +%s%N)"
  if [[ -n "${cmd}" ]]; then
    profile_tag.trap_eval "${tag_id}" "${cmd}"
  fi
  profile_tag.digest "${tag_id}" "${digest_args[@]}"
}
