...
fn_arg_info "h-deploy" "Deploys a built repository from an app directory to a caprover server via a tar file."
fn_arg_accept 'r:' 'app-dir' 'The output folder for a build that also contains any app specific env files.'
fn_arg_accept 'd:' 'dist-dir' 'The dir of distribution files generated via the build script'
fn_arg_parse "$@"

app_dir="$(fn_get_arg 'app-dir')"
dist_dir="$(fn_get_arg 'dist-dir')"
