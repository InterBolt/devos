...
# Ie. collection.main, push.main, process.main.
. "${HOME}/.solos/src/bash/container/daemon-scrub.sh" || exit 1
. "${HOME}/.solos/src/bash/container/daemon-phase-pull.sh" || exit 1
. "${HOME}/.solos/src/bash/container/daemon-phase-collection.sh" || exit 1
. "${HOME}/.solos/src/bash/container/daemon-phase-process.sh" || exit 1
...
daemon_main.run() {
  ...
  while true; do
    ...
    if ! daemon_phase_pull.main "${pulled_data_dir}" "${daemon_main__phase_cache}"; then
      ...
      daemon_main.log_info "Lifecycle - collection ran successfully."
    fi
    # ------------------------------------------------------------------------------------
    #
    # COLLECTOR PHASE:
    ...
    #
    # ------------------------------------------------------------------------------------
    if ! daemon_phase_collection.main "${scrubbed_volume_dir}" "${pulled_data_dir}" "${collections_dir}"; then
      if [[ $? -eq 151 ]]; then
        daemon_main.handle_phase_kill_request "collection"
        return "$?"
      fi
      daemon_main.log_error "Lifecycle - collection ${phase_kill_request_message}"
    else
      daemon_main.log_info "Lifecycle - collection ran successfully."
    fi
    # ------------------------------------------------------------------------------------
    ...
    #
    # ------------------------------------------------------------------------------------
    if ! daemon_phase_process.main "${collections_dir}" >"${processed_file}"; then
      if [[ $? -eq 151 ]]; then
        daemon_main.handle_phase_kill_request "process"
        return "$?"
      fi
      daemon_main.log_error "Lifecycle - process ${phase_kill_request_message}"
    else
      daemon_main.log_info "Lifecycle - process ran successfully."
    fi
    # ------------------------------------------------------------------------------------
    #
    # PUSH PHASE:
